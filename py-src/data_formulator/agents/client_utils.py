...
def _normalize_ollama_base(api_base: str | None) -> str:
    """Normalize Ollama base URL to scheme://host[:port] (no path)."""
    raw = (api_base or "http://localhost:11434").strip()
    parsed = urlparse(raw)
...
class Client(object):
    ...
    def __init__(self, endpoint, model, api_key=None,  api_base=None, api_version=None):
        ...
        if self.endpoint == "gemini":
            ...
            self.params["api_base"] = _normalize_ollama_base(api_base)
