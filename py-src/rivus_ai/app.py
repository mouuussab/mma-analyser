...
app.config['CLI_ARGS'] = {
    ...
    'data_dir': os.environ.get('RIVUS_HOME', None),
    ...
    'azure_blob_container': os.environ.get('AZURE_BLOB_CONTAINER', 'rivus'),
}
...
def configure_logging():
    ...
    logging.getLogger('rivus_ai.agents').setLevel(logging.INFO)
...
def _register_blueprints():
    ...
    from rivus_ai.tables_routes import tables_bp
    
    # Import agent routes (imports AI/ML libraries: litellm, sklearn, etc.)
    print("  Loading AI agents...", flush=True)
    from rivus_ai.agent_routes import agent_bp
    
    # Import session routes
    from rivus_ai.session_routes import session_bp

    # Import demo stream routes
    from rivus_ai.demo_stream_routes import demo_stream_bp, limiter as demo_stream_limiter, start_iss_collector
...
def get_sample_datasets():
    from rivus_ai.example_datasets_config import EXAMPLE_DATASETS
...
def get_app_config():
    ...
    if not args['disable_database']:
        ...
        if workspace_backend != 'azure_blob':
            from rivus_ai.datalake.workspace import get_rivus_home
            config["RIVUS_HOME"] = str(get_rivus_home())
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--data-dir", type=str, default=None,
        help="RIVUS home directory for workspaces and sessions (default: ~/.rivus)")
    ...
    parser.add_argument("--azure-blob-container", type=str,
        default=os.environ.get('AZURE_BLOB_CONTAINER', 'rivus'),
        help="Azure Blob Storage container name (default: rivus)")
