...
def get_rivus_home() -> Path:
    """Get the RIVUS home directory.

    Resolution order:
    1. Flask app.config['CLI_ARGS']['data_dir'] (set via --data-dir CLI flag)
    2. RIVUS_HOME environment variable
    3. Default: ~/.rivus
    """
...
def get_default_workspace_root() -> Path:
    ...
    Returns RIVUS_HOME / "workspaces".
    """
    return get_rivus_home() / "workspaces"
...
class Workspace:
    ...
    def _get_sessions_root(self) -> Path:
        ...
        p = get_rivus_home() / "sessions" / self._safe_id
