...
from rivus_ai.agents.agent_utils import extract_json_objects, generate_data_summary
...
class InteractiveExploreAgent(object):
    ...
    def __init__(self, client, workspace, agent_exploration_rules="", response_language="en"):
        self.client = client
        ...
        self.response_language = (response_language or "en").lower()
    ...
    def run(self, input_tables, start_question=None, exploration_thread=None, 
        ...
        # Add language instruction at the very beginning for better effect
        if self.response_language.startswith("fr"):
            language_instruction = (
                "⚠️ IMPORTANT: You MUST respond entirely in French.\n"
                "- All output text fields (text, goal, questions, tags, thoughts) MUST be in French\n"
        ...
        if self.response_language.startswith("fr"):
            system_prompt += (
                ...
                "Remember: ALL output must be in French. Check each word."
            )
