...
export const ModelSelectionButton: React.FC<{}> = ({ }) => {
    ...
    const text = appLanguage === 'fr' ? {
        ready: 'Pret',
        retest: 'Retester',
        test: 'Tester',
        removeModel: 'supprimer le modele',
        pleaseSelectModel: 'Veuillez selectionner un modele',
        unselected: 'Non selectionne',
        selectModel: 'Selectionner un modele',
        selectModels: 'Selectionner des modeles',
        modelsWithStrongCode: 'Les modeles avec une forte capacite de generation de code (ex:',
        provideBestExperience: ') offrent la meilleure experience.',
        modelConfigLiteLLM: 'Configuration des modeles basee sur LiteLLM.',
        seeSupportedProviders: 'Voir les fournisseurs pris en charge',
        useOpenAIProvider: 'Utilisez le fournisseur',
        forOpenAICompatible: 'pour les API compatibles OpenAI.',
        hide: 'masquer',
        show: 'afficher',
        keys: 'cles',
        use: 'Utiliser',
        ...
        modelsWithStrongCode: 'Models with strong code generation capabilities (e.g.,',
        provideBestExperience: ') provide best experience.',
        modelConfigLiteLLM: 'Model configuration based on LiteLLM.',
        seeSupportedProviders: 'See supported providers',
        useOpenAIProvider: 'Use',
        forOpenAICompatible: 'provider for OpenAI-compatible APIs.',
        hide: 'hide',
        show: 'show',
        keys: 'keys',
        use: 'Use',
    ...
    let testModel = (model: ModelConfig) => {
        ...
        fetchWithIdentity(getUrls().TEST_MODEL, {...message })
            .then(async (response) => {
                ...
                if (!raw || !raw.trim()) {
                    throw new Error(appLanguage === 'fr'
                        ? "Le backend est indisponible. Verifiez que le serveur Python tourne sur le port 5567."
                        : "Backend is unavailable. Make sure the Python server is running on port 5567.");
                }
                try {
                    return JSON.parse(raw);
                } catch {
                    throw new Error(appLanguage === 'fr'
                        ? "Reponse invalide du backend pendant le test du modele."
                        : "Invalid backend response while testing model.");
                }
            })
            .then((data) => {
        ...
        <Table sx={{ minWidth: 600, "& .MuiTableCell-root": { padding: "4px 8px", borderBottom: "none", fontSize: '0.75rem' } }} size="small" >
            ...
            <TableBody>
                {models.map((model) => {
                    ...
                    const borderStyle = ['error'].includes(status) ? `1px dashed ${theme.palette.divider}` : undefined;
                    ...
                    return (
                        ...
                        <TableRow
                            ...
                            sx={{ 
                                ...
                                '&:hover': { backgroundColor: theme.palette.mode === 'dark' ? alpha(theme.palette.primary.main, 0.15) : '#f8f9fa' },
                        ...
                        >
                            ...
                            <TableCell sx={{borderBottom: borderStyle}} align="left">
                                <Tooltip title={message}>
                                    ...
                                    >
                                        {status == 'ok' ? text.ready : status == 'error' ? text.retest : text.test}
                                    </Button>
                                </Tooltip>
                            </TableCell>
                            <TableCell sx={{ borderBottom: borderStyle }} align="right">
                                <Tooltip title={text.removeModel}>
                                    <IconButton 
                                        size="small"
                                        onClick={()=>{
    ...
    let tempModelName = tempModel ? `${tempModel.endpoint}/${tempModel.model}` : text.pleaseSelectModel;
    let selectedModelName = models.find(m => m.id == selectedModelId)?.model || text.unselected;

    return <>
        <Tooltip title={text.selectModel}>
            <Button sx={{fontSize: "inherit", textTransform: "none"}} variant="text" color={modelNotReady ? 'warning' : "primary"} onClick={()=>{setModelDialogOpen(true)}}>
                {modelNotReady ? text.selectModels : selectedModelName}
            </Button>
        </Tooltip>
        <Dialog 
            maxWidth="lg" 
        ...
        >
            <DialogTitle sx={{display: "flex",  alignItems: "center"}}>{text.selectModel}</DialogTitle>
            <DialogContent >
            <Box sx={{
                    display: 'flex', 
                    ...
                    <Box>
                        <Typography variant="caption" component="div" sx={{ lineHeight: 1.6 }}>
                            • {text.modelsWithStrongCode} <code style={{ 
                                ...
                                borderRadius: 3,
                                fontSize: '0.7rem'
                            }}>claude-sonnet-4-5</code>{text.provideBestExperience}
                        </Typography>
                        <Typography variant="caption" component="div" sx={{ lineHeight: 1.6, mt: 0.5 }}>
                            • {text.modelConfigLiteLLM} <a href="https://docs.litellm.ai/docs/" target="_blank" rel="noopener noreferrer">{text.seeSupportedProviders}</a>.
                            {text.useOpenAIProvider} <code style={{ 
                                backgroundColor: alpha(theme.palette.text.primary, 0.05), 
                                padding: '1px 4px', 
                                borderRadius: 3,
                                fontSize: '0.7rem'
                            }}>openai</code> {text.forOpenAICompatible}
                        </Typography>
            ...
            <DialogActions>
                {!serverConfig.DISABLE_DISPLAY_KEYS && (
                    <Button sx={{marginRight: 'auto'}} endIcon={showKeys ? <VisibilityOffIcon /> : <VisibilityIcon />} onClick={()=>{
                        setShowKeys(!showKeys);}}>
                            {showKeys ? text.hide : text.show} {text.keys}
                    </Button>
                )}
                <Button disabled={modelNotReady} sx={{textTransform: 'none'}}
                    ...
                    onClick={()=>{
                        ...
                        setModelDialogOpen(false);}}>{text.use} {tempModelName}</Button>
                <Button onClick={()=>{
