...
export const DatasetSelectionView: React.FC<DatasetSelectionViewProps> = function DatasetSelectionView({ datasets, handleSelectDataset, hideRowNum  }) {
    ...
    const text = appLanguage === 'fr' ? {
        ...
        loadDataset: 'charger le jeu de donnees',
    } : {
        from: 'from',
                        ...
                        <Box>
                            <Box sx={{mb: 1, gap: 1, maxWidth: 800, display: "flex", alignItems: "center", flexWrap: "wrap"}}>
                                <Typography sx={{fontSize: 12, flex: 1, minWidth: 200}}>
                                    {selectedDataset.description} <Typography variant="caption" sx={{color: "primary.light", fontSize: 10, mx: 0.5}}>[{text.from} {selectedDataset.source}]</Typography>
                                </Typography>
                            </Box>
                            <Box sx={{ maxWidth: 800 }}>
                                <MultiTablePreview
                                    tables={previewTables}
                                    emptyLabel={text.noTables}
