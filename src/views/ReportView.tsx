...
const reportText: Record<'en' | 'fr', {
    captureNotFound: string;
    failedGenerateImage: string;
    copiedImage: string;
    failedCopyClipboard: string;
    clipboardUnsupported: string;
    failedGenerateReportImage: string;
    selectAtLeastOneChart: string;
    noModelSelected: string;
    failedGenerateReport: string;
    noResponseBody: string;
    backToExplore: string;
    viewReports: string;
    createA: string;
    from: string;
    chartSingular: string;
    chartPlural: string;
    composing: string;
    compose: string;
    noChartsAvailable: string;
    loadingChartPreviews: string;
    noChartsToDisplay: string;
    ...
    copied: string;
    shareImage: string;
    createdWithAI: string;
}> = {
    en: {
        captureNotFound: 'Could not find report content to capture',
        failedGenerateImage: 'Failed to generate image',
        copiedImage: 'Report image copied to clipboard! You can now paste it anywhere to share.',
        failedCopyClipboard: 'Failed to copy to clipboard. Your browser may not support this feature.',
        clipboardUnsupported: 'Clipboard API not supported in your browser. Please use a modern browser.',
        failedGenerateReportImage: 'Failed to generate report image. Please try again.',
        ...
        composing: 'composing...',
        compose: 'compose',
        noChartsAvailable: 'No charts available. Create some visualizations first.',
        loadingChartPreviews: 'loading chart previews...',
        noChartsToDisplay: 'No available charts to display. Charts may still be loading or unavailable.',
        createNewReport: 'create a new report',
        aiWarning: 'AI generated the post from the selected charts, and it could be inaccurate!',
    ...
    fr: {
        captureNotFound: "Impossible de trouver le contenu du rapport a capturer",
        failedGenerateImage: "Echec de la generation de l'image",
        copiedImage: "Image du rapport copiee dans le presse-papiers. Vous pouvez maintenant la coller pour la partager.",
        failedCopyClipboard: "Echec de la copie dans le presse-papiers. Votre navigateur ne prend peut-etre pas en charge cette fonctionnalite.",
        clipboardUnsupported: "L'API presse-papiers n'est pas prise en charge par votre navigateur. Utilisez un navigateur moderne.",
        failedGenerateReportImage: "Echec de la generation de l'image du rapport. Reessayez.",
        selectAtLeastOneChart: 'Veuillez selectionner au moins un graphique',
        noModelSelected: 'Aucun modele selectionne',
        failedGenerateReport: 'Echec de la generation du rapport',
        noResponseBody: 'Aucun contenu de reponse',
        backToExplore: "retour a l'exploration",
        viewReports: 'voir les rapports',
        createA: 'Creer un',
        from: 'a partir de',
        chartSingular: 'graphique',
        chartPlural: 'graphiques',
        composing: 'generation...',
        compose: 'generer',
        noChartsAvailable: "Aucun graphique disponible. Creez d'abord des visualisations.",
        loadingChartPreviews: 'chargement des apercus...',
        noChartsToDisplay: "Aucun graphique disponible a afficher. Les graphiques sont peut-etre encore en chargement ou indisponibles.",
        createNewReport: 'creer un nouveau rapport',
        aiWarning: "Le texte genere par l'IA a partir des graphiques selectionnes peut etre inexact.",
        ...
        createdWithAI: "cree avec l'IA",
    },
};

const createDarkMarkdownOverrides = (theme: any) => ({
    ...getOverrides(),
    h1: { component: Typography, props: { variant: 'h4', gutterBottom: true, sx: { color: 'text.primary' } } },
    h2: { component: Typography, props: { variant: 'h5', gutterBottom: true, sx: { color: 'text.primary' } } },
    h3: { component: Typography, props: { variant: 'h6', gutterBottom: true, sx: { color: 'text.primary' } } },
    h4: { component: Typography, props: { variant: 'subtitle1', gutterBottom: true, sx: { color: 'text.primary' } } },
    p: { component: Typography, props: { variant: 'body2', paragraph: true, sx: { color: 'text.primary' } } },
    a: { component: Link, props: { underline: 'hover' as const, sx: { color: theme.palette.primary.light } } },
    li: { component: Typography, props: { component: 'li', variant: 'body2', sx: { color: 'text.primary' } } },
    blockquote: { component: Box, props: { sx: { borderLeft: '3px solid', borderColor: borderColor.divider, pl: 2, py: 1, backgroundColor: alpha(theme.palette.action.hover, 0.3), color: 'text.secondary' } } },
    pre: { component: Paper, props: { elevation: 0, sx: { backgroundColor: alpha(theme.palette.background.default, 0.65), p: 2, border: `1px solid ${borderColor.divider}`, '& code': { color: 'text.primary' } } } },
    table: { component: TableContainer, props: { component: Paper, elevation: 0, sx: { my: 2, border: `1px solid ${borderColor.divider}` } } },
    thead: { component: TableHead, props: { sx: { backgroundColor: alpha(theme.palette.action.hover, 0.25) } } },
    tbody: { component: TableBody },
    tr: { component: TableRow },
    th: { component: TableCell, props: { sx: { color: 'text.primary', borderBottom: `1px solid ${borderColor.divider}` } } },
    td: { component: TableCell, props: { sx: { color: 'text.primary', borderBottom: `1px solid ${borderColor.divider}` } } },
    hr: { component: Divider, props: { sx: { my: 2, borderColor: borderColor.divider } } },
}) as any;
...
export const ReportView: FC = () => {
    ...
    const darkMarkdownOverrides = useMemo(() => createDarkMarkdownOverrides(theme), [theme]);
    const styleLabels = useMemo(() => ({
        'live report': language === 'fr' ? 'rapport live' : 'live report',
        'blog post': language === 'fr' ? 'article' : 'blog post',
        'social post': language === 'fr' ? 'post social' : 'social post',
        'executive summary': language === 'fr' ? 'synthese executive' : 'executive summary',
    }), [language]);
    ...
    const shareReportAsImage = async () => {
        ...
        try {
            ...
            if (!reportElement) {
                showMessage(t.captureNotFound, 'error');
                return;
            }
            ...
            canvas.toBlob((blob: Blob | null) => {
                if (!blob) {
                    showMessage(t.failedGenerateImage, 'error');
                    return;
                }
                ...
                if (navigator.clipboard && navigator.clipboard.write) {
                    navigator.clipboard.write([
                        ...
                        showMessage(t.copiedImage);
                        setShareButtonSuccess(true);
                        setTimeout(() => setShareButtonSuccess(false), 2000);
                    }).catch(() => {
                        showMessage(t.failedCopyClipboard, 'error');
                    });
                } else {
                    showMessage(t.clipboardUnsupported, 'error');
                }
            }, 'image/png', 0.95);

        } catch (error) {
            console.error('Error generating report image:', error);
            showMessage(t.failedGenerateReportImage, 'error');
        }
    };
    ...
    const generateReport = async () => {
        if (selectedChartIds.size === 0) {
            setError(t.selectAtLeastOneChart);
            return;
        }
        ...
        try {
            ...
            if (!response.ok) {
                throw new Error(t.failedGenerateReport);
            }
            ...
            while (true) {
                ...
                if (chunk.startsWith('error:')) {
                    ...
                    throw new Error(errorData.content || t.failedGenerateReport);
                }
            ...
            setError((err as Error).message || t.failedGenerateReport);
        } finally {
                        ...
                        >
                            ...
                            >
                                {[
                                    ...
                                    >
                                        {option.value === 'live report' ? <StreamIcon sx={{ fontSize: 16, mr: 1 }} /> : <></>} {styleLabels[option.value as keyof typeof styleLabels]}
                                    </ToggleButton>
                                ))}
                            </ToggleButtonGroup>
                            ...
                            <Typography variant="body2" color="text.primary" sx={{ fontWeight: 500 }}>
                                {selectedChartIds.size <= 1 ? t.chartSingular : t.chartPlural}
                            </Typography>
                            ...
                            >
                                {isGenerating ? t.composing : t.compose}
                            </Button>
                        </Paper>
                    </Box>
                                    ...
                                    {availableCharts.map((chart) => {
                                        ...
                                        >
                                            <Box sx={{ position: 'relative' }}>
                                                <Checkbox
                                                    ...
                                                    sx={{ 
                                                        ...
                                                        backgroundColor: alpha(theme.palette.background.paper, 0.9), borderRadius: 1,
                                                ...
                                                <Box
                                                    ...
                                                    alt={getLocalizedChartType(chart.chartType, language)}
                                                    sx={{ p: 1, width: `calc(100% - 16px)`, height: 'auto', maxHeight: config.defaultChartHeight, display: 'block', objectFit: 'contain', backgroundColor: 'background.paper' }}
                                                />
                                            </Box>
                                            <CardContent sx={{ p: 1, '&:last-child': { pb: 1.5 } }}>
                                                ...
                                                >
                                                    {getLocalizedChartType(chart.chartType, language)}
                                                </Typography>
                                                {table?.displayId && (
                                                    <Typography 
                                                        variant="caption" 
                        ...
                        {allGeneratedReports.length > 0 && (
                            <Box sx={{ 
                                ...
                                sx={{
                                    ...
                                    : <ExpandLessIcon sx={{ fontSize: 16, mr: 1 }} /> } {hideTableOfContents ? t.showAllReports : t.reports}
                                </Button> 
                            ...
                            {currentReportId && (
                                <Box sx={{ position: 'absolute', top: 16, right: 16, zIndex: 10, display: 'flex', gap: 1 }}>
                                    <Tooltip title={t.shareReportAsImage}>
                                        ...
                                        >
                                            {shareButtonSuccess ? t.copied : t.shareImage}
                                        </Button>
                                    </Tooltip>
                                </Box>
                            )}
                            ...
                            <Box sx={{ display: 'flex', justifyContent: 'center', width: '100%', py: 3 }}>
                                <Box
                                    ...
                                    sx={{
                                        ...
                                        // Conditional styles
                                        ...(isDarkMode ? {
                                            maxWidth: (generatedStyle === 'social post' || generatedStyle === 'short note')
                                                ? '520px'
                                                : generatedStyle === 'executive summary'
                                                    ? '700px'
                                                    : '800px',
                                            borderRadius: generatedStyle === 'social post' || generatedStyle === 'short note' ? '12px' : 0,
                                            border: generatedStyle === 'social post' || generatedStyle === 'short note' ? '1px solid' : undefined,
                                            borderColor: borderColor.divider,
                                            p: generatedStyle === 'social post' || generatedStyle === 'short note' || generatedStyle === 'executive summary' ? 2.5 : undefined,
                                            px: generatedStyle === 'social post' || generatedStyle === 'short note' || generatedStyle === 'executive summary' ? undefined : 6,
                                            py: generatedStyle === 'social post' || generatedStyle === 'short note' || generatedStyle === 'executive summary' ? undefined : 0,
                                            ...
                                            lineHeight: 1.5,
                                            '& code': {
                                                backgroundColor: alpha(theme.palette.action.selected, 0.45),
                                                color: 'text.primary',
                                                padding: '0.2em 0.4em',
                                                borderRadius: '3px',
                                                fontSize: '0.8125rem',
                                            ...
                                            '& img': {
                                                maxWidth: generatedStyle === 'social post' || generatedStyle === 'short note' ? '100%' : '75%',
                                                maxHeight: generatedStyle === 'social post' || generatedStyle === 'short note' ? '280px' : config.defaultChartHeight * 1.5,
                                                objectFit: 'contain',
                                                width: 'auto',
                                                height: 'auto',
                                                borderRadius: '4px',
                                                marginTop: generatedStyle === 'social post' || generatedStyle === 'short note' ? '8px' : '1.25em',
                                                marginBottom: generatedStyle === 'social post' || generatedStyle === 'short note' ? '8px' : '1.25em',
                                            }
                                        } : generatedStyle === 'social post' || generatedStyle === 'short note' ? {
                                            maxWidth: '520px', borderRadius: '12px',
                                            border: '1px solid', borderColor: COLOR_SOCIAL_BORDER, p: 2.5, backgroundColor: 'background.paper',
                                            ...
                                            maxWidth: '700px', p: 2.5, backgroundColor: 'background.paper',
