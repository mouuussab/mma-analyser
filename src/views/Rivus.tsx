...
interface RivusProps {
    language?: AppLanguage;
}

const pageText: Record<AppLanguage, {
    subtitle: string;
    firstModel: string;
    modelTip: string;
}> = {
...
export const RivusFC = ({ language = "en" }: RivusProps) => {
    ...
    const fixedSplitPane = ( 
        <Box sx={{display: 'flex', flexDirection: 'row', height: '100%'}}>
            <Box ref={containerRef} className="outer-allotment" sx={{
                    margin: '4px 8px 8px 8px', backgroundColor: theme.palette.background.paper,
    ...
    let footer = <Box sx={{ color: 'text.secondary', display: 'flex',
            backgroundColor: alpha(theme.palette.background.paper, 0.89),
            alignItems: 'center', justifyContent: 'center', py: 0.5 }}>
        <Typography sx={{ display: 'inline', fontSize: '12px', ml: 1 }}>
            © {new Date().getFullYear()} {toolName}
        </Typography>
    </Box>
    ...
    let dataUploadRequestBox = <Box sx={{
        ...
        <Box sx={{margin:'auto', pb: '5%', display: "flex", flexDirection: "column", textAlign: "center" }}>
            <Box sx={{display: 'flex', mx: 'auto', mb: 2}}>
                <Box component="img" src="/rivus-logo-full.svg" alt="RIVUS Logo" sx={{maxWidth: 400, height: 'auto'}} />
            </Box>
            <Typography sx={{ 
            ...
            <DndProvider backend={HTML5Backend}>
                ...
                {selectedModelId == undefined && (
                    <Box sx={{
                        ...
                        <Box sx={{margin:'auto', pb: '5%', display: "flex", flexDirection: "column", textAlign: "center"}}>
                            <Typography variant="h3" sx={{ fontWeight: 200, letterSpacing: '0.05em' }}>
                                {toolName}
                            </Typography>
                            <Typography  variant="h4" sx={{mt: 3, fontSize: 28, letterSpacing: '0.02em'}}>
                                {text.firstModel} <ModelSelectionButton />
                            </Typography>
                            <Typography  color="text.secondary" variant="body1" sx={{mt: 2, width: 600}}>💡 {text.modelTip}</Typography>
                        </Box>
                        {footer}
                    </Box>
                )}
