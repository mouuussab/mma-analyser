...
const DraggableHeader: React.FC<DraggableHeaderProps> = ({ 
    ...
    return (
        ...
        >
            ...
            <Tooltip title={<Typography sx={{fontSize: 10}}>{appLanguage === 'fr' ? 'Trier par' : 'Sort by'} <b>{columnDef.label}</b></Typography>}>
                <IconButton
                    size="small"
                    onClick={(e) => {
                        e.stopPropagation();
...
export const SelectableDataGrid: React.FC<SelectableDataGridProps> = ({ 
    ...
    const text = appLanguage === 'fr' ? {
        loading: 'Chargement...',
        rows: 'lignes',
        randomRows: 'voir 10000 lignes aleatoires de cette table',
        downloadCsv: 'Telecharger en CSV',
    } : {
        loading: 'Loading...',
        rows: 'rows',
        randomRows: 'view 10000 random rows from this table',
        downloadCsv: 'Download as CSV',
    };
    const evenBg = theme.palette.mode === 'dark' ? alpha(theme.palette.common.white, 0.04) : alpha(theme.palette.common.black, 0.03);
    const oddBg = theme.palette.mode === 'dark' ? alpha(theme.palette.common.white, 0.02) : theme.palette.background.paper;
    const hoverBg = theme.palette.mode === 'dark' ? alpha(theme.palette.primary.light, 0.12) : alpha(theme.palette.primary.main, 0.08);
    ...
    const TableComponents = {
        Scroller: React.forwardRef<HTMLDivElement>((props, ref) => (
            <TableContainer {...props} ref={ref} sx={{ backgroundColor: theme.palette.background.paper }} />
        )),
        ...
        <Box className="table-container table-container-small"
            ...
            {isLoading && (
                <Box sx={{ 
                    ...
                    backgroundColor: theme.palette.mode === 'dark' ? 'rgba(17, 24, 39, 0.85)' : 'rgba(255, 255, 255, 0.7)',
                    padding: '8px',
                    height: '100%',
                    borderTopLeftRadius: '4px',
                    borderTopRightRadius: '4px'
                }}>
                    <CircularProgress size={24} sx={{ mr: 1, color: theme.palette.mode === 'dark' ? theme.palette.primary.light : theme.palette.primary.main }} />
                    <Typography variant="body2" color="text.secondary">{text.loading}</Typography>
                </Box>
            )}
            ...
            <Paper variant="outlined"
                ...
                <Box sx={{display: 'flex', alignItems: 'center', mx: 1}}>
                    ...
                    <Tooltip title={text.downloadCsv}>
                        <IconButton 
                            size="small" 
                            color="primary" 
