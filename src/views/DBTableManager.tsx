...
const dbManagerText: Record<'en' | 'fr', {
    failedDownload: string;
    failedFetchTables: string;
    failedFetchParams: string;
    selectDataLoader: string;
    selectedDataLoaderUnavailable: string;
    subsetLoaded: string;
    loaded: string;
    failedLoadData: string;
    failedFetchDataLoaderTables: string;
    failedLoadTable: string;
    tableFilter: string;
    connect: string;
}> = {
    en: {
        failedDownload: 'Failed to download database file',
        failedFetchTables: 'Failed to fetch tables, please check if the server is running',
        failedFetchParams: 'Failed to fetch data loader params',
        selectDataLoader: 'Select a data loader from the left panel',
        selectedDataLoaderUnavailable: 'Selected data loader is unavailable',
        subsetLoaded: 'Subset loaded',
        loaded: 'Loaded',
        failedLoadData: 'Failed to load data',
        failedFetchDataLoaderTables: 'Failed to fetch data loader tables',
        failedLoadTable: 'Failed to load table',
        tableFilter: 'table filter',
        connect: 'Connect',
    },
    fr: {
        failedDownload: 'Échec du téléchargement du fichier de base de données',
        failedFetchTables: 'Impossible de récupérer les tables, vérifiez que le serveur est en cours d\'exécution',
        failedFetchParams: 'Impossible de récupérer les paramètres du chargeur de données',
        selectDataLoader: 'Sélectionnez un chargeur de données dans le panneau de gauche',
        selectedDataLoaderUnavailable: 'Le chargeur de données sélectionné est indisponible',
        subsetLoaded: 'Sous-ensemble chargé',
        loaded: 'Chargé',
        failedLoadData: 'Impossible de charger les données',
        failedFetchDataLoaderTables: 'Impossible de récupérer les tables du chargeur de données',
        failedLoadTable: 'Impossible de charger la table',
...
export const DBManagerPane: React.FC<{ 
    ...
    const text = dbManagerText[appLanguage];
    ...
    const fetchTables = async (): Promise<DBTable[] | undefined> => {
        try {
            ...
            setSystemMessage(text.failedFetchTables, "error");
        }
        return undefined;
    };
    ...
    let dataConnectorView = <Box sx={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, overflowY: 'auto', overflowX: 'hidden', p: 2, pb: 4, display: 'flex', flexDirection: 'column', minWidth: 0, overscrollBehavior: 'contain' }}>
        ...
        {selectedDataLoader === '' && (
            <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', flex: 1, color: 'text.disabled' }}>
                <Typography variant="body2" sx={{ fontStyle: 'italic' }}>
                    {text.selectDataLoader}
                </Typography>
            </Box>
        )}
        
        {selectedDataLoader !== '' && !dataLoaderMetadata[selectedDataLoader] && (
            <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', flex: 1, color: 'text.disabled' }}>
                <Typography variant="body2" sx={{ fontStyle: 'italic' }}>
                    {text.selectedDataLoaderUnavailable}
                </Typography>
            </Box>
        )}
        ...
        {/* Data loader form */}
        {selectedDataLoader !== '' && dataLoaderMetadata[selectedDataLoader] && (
            <Box key={`dataLoader:${selectedDataLoader}`} sx={{ position: "relative", maxWidth: '100%', flexShrink: 0, width: '100%' }}>
                <DataLoaderForm
                    key={`data-loader-form-${selectedDataLoader}`}
                    dataLoaderType={selectedDataLoader}
                    paramDefs={dataLoaderMetadata[selectedDataLoader].params}
                    authInstructions={dataLoaderMetadata[selectedDataLoader].auth_instructions}
    ...
    let mainContent =  
        <Box sx={{ display: 'flex', flexDirection: 'column', bgcolor: theme.palette.background.paper, flex: 1, overflow: 'hidden', height: '100%' }}>
        ...
        <Box sx={{ position: 'relative', display: 'flex', flexDirection: 'column', height: '100%', width: '100%' }}>
            ...
            {isUploading && (
                <Box sx={{ 
                    ...
                    backgroundColor: theme.palette.mode === 'dark' ? 'rgba(30, 30, 30, 0.95)' : 'rgba(255, 255, 255, 0.85)',
...
export const DataLoaderForm: React.FC<{
    ...
    const safeAuthInstructions = typeof authInstructions === 'string' ? authInstructions : '';
                    ...
                    <Box>
                        ...
                        <Box sx={{ mt: 1.5, pt: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 2, flexWrap: 'nowrap' }}>
                            ...
                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5, flexShrink: 0 }}>
                                ...
                                >
                                    {effectiveLoadedTables[selectedPreviewTable] === 'subset' ? text.subsetLoaded : text.loaded}
                                </Button>
                                <Button
                                    variant="text"
                                    size="small"
                                    ...
                                    sx={{ textTransform: 'none', fontSize: 11, px: 1, minWidth: 0, height: 28, color: 'text.secondary',
                                        '&:hover': { color: 'error.main', backgroundColor: theme.palette.mode === 'dark' ? alpha(theme.palette.error.main, 0.12) : 'rgba(211,47,47,0.04)' },
                                    }}
                                >
                                    Unload
                                </Button>
                ...
                <Box sx={{}}>
                    <Box sx={{mb: 1.5}}>
                        <Box sx={{display: "flex", flexDirection: "row", alignItems: "center", gap: 0.5, mb: 1.5, flexWrap: "wrap"}}>
                            ...
                            {safeParamDefs.filter((paramDef) => params[paramDef.name]).length > 0 && (
                                <Typography variant="body2" component="span" sx={{fontSize: 11, color: 'text.disabled', mr: 0.5}}>·</Typography>
                            )}
                            {safeParamDefs.filter((paramDef) => params[paramDef.name]).map((paramDef, index) => (
                                <React.Fragment key={paramDef.name}>
                                    ...
                                    {index < safeParamDefs.filter((p) => params[p.name]).length - 1 && (
                                        <Typography variant="body2" component="span" sx={{fontSize: 11, color: 'text.disabled', mr: 0.5}}>·</Typography>
                                    )}
                                </React.Fragment>
                            ))}
                        ...
                        <Box sx={{display: "flex", flexDirection: "row", alignItems: "flex-end", gap: 1.5}}>
                            <Box sx={{ display: "flex", flexDirection: "column", gap: 0.25, flex: 1, maxWidth: 300 }}>
                                <Typography sx={{ fontSize: 11, fontWeight: 500, color: 'text.secondary', lineHeight: 1.3, display: 'flex', alignItems: 'center', gap: 0.5 }}>
                                    <SearchIcon sx={{ fontSize: 11 }} /> {text.tableFilter}
                                </Typography>
                                <TextField
                                    sx={{
                                    ...
                                    fullWidth
                                    autoComplete="off"
                                    placeholder={appLanguage === 'fr' ? 'filtrer les tables par mot-clé' : 'filter tables by keyword'}
                                    value={tableFilter}
                ...
                <>
                    ...
                    {safeAuthInstructions.trim() && (
                        <Box sx={(theme) => ({
                            ...
                            <Markdown>{safeAuthInstructions.trim()}</Markdown>
                        </Box>
                    )}</>
            )}
        </Box>
