...
function TabPanel(props: TabPanelProps) {
    ...
    return (
        <Box
            ...
            sx={{ height: '100%', overflow: 'auto', boxSizing: 'border-box', backgroundColor: theme.palette.background.paper }}
            {...other}
        >
...
export const DataLoadMenu: React.FC<DataLoadMenuProps> = ({ 
    ...
    const t = appLanguage === 'fr' ? {
        sampleDatasets: 'Jeux de donnees exemples',
        sampleDatasetsDesc: 'Explorer et charger des jeux de donnees exemples',
        uploadFile: 'Telecharger un fichier',
        uploadFileDesc: 'Telecharger des fichiers locaux (CSV, TSV, JSON, Excel)',
        pasteData: 'Coller des donnees',
        pasteDataDesc: 'Coller des donnees tabulaires depuis le presse-papiers',
        extractData: 'Extraire des donnees non structurees',
        extractDataDesc: "Extraire des tableaux depuis des images ou du texte avec l'IA",
        loadFromUrl: 'Charger depuis une URL',
        loadFromUrlDesc: 'Charger des donnees depuis une URL avec rafraichissement optionnel',
        database: 'Base de donnees',
        databaseDesc: 'Se connecter aux bases de donnees ou services de donnees',
        connectLive: 'Se connecter aux sources de donnees en direct',
        loadLocalData: 'Charger des donnees locales',
        localData: 'Donnees locales',
        connectSource: 'Ou se connecter a une source de donnees (rafraichissement optionnel)',
    } : {
        sampleDatasets: 'Sample Datasets',
        sampleDatasetsDesc: 'Explore and load curated example datasets',
        uploadFile: 'Upload File',
        uploadFileDesc: 'Upload local files (CSV, TSV, JSON, Excel)',
        pasteData: 'Paste Data',
        pasteDataDesc: 'Paste tabular data directly from clipboard',
        extractData: 'Extract Unstructured Data',
        extractDataDesc: 'Extract tables from images or text using AI',
        loadFromUrl: 'Load from URL',
        loadFromUrlDesc: 'Load data from a URL with optional auto-refresh',
        database: 'Database',
        databaseDesc: 'Connect to databases or data services',
        connectLive: 'Connect to live data sources',
        loadLocalData: 'Load local data',
        localData: 'Local data',
        connectSource: 'Or connect to a data source (with optional auto-refresh)',
    };
    ...
    const regularDataSources = [
        { 
            ...
            description: t.sampleDatasetsDesc,
            icon: <ExploreIcon />, 
            disabled: false
        },
        { 
...
export const UnifiedDataUploadDialog: React.FC<UnifiedDataUploadDialogProps> = ({
    ...
    const theme = useTheme();
    const appLanguage = useAppLanguage();
    const ui = appLanguage === 'fr' ? {
        loadData: 'Charger des donnees',
        sampleDatasets: 'Jeux de donnees exemples',
        uploadFile: 'Telecharger un fichier',
        pasteData: 'Coller des donnees',
        extractFromDocuments: 'Extraire depuis des documents',
        loadFromUrl: 'Charger depuis une URL',
        database: 'Base de donnees',
        addData: 'Ajouter des donnees',
        resetExtraction: "Reinitialiser l'extraction",
        loadDataIn: 'Charger les donnees dans',
        browser: 'Navigateur',
        disk: 'Disque',
        openWorkspace: 'Ouvrir espace de travail',
    } : {
        loadData: 'Load Data',
        sampleDatasets: 'Sample Datasets',
        uploadFile: 'Upload File',
        pasteData: 'Paste Data',
        extractFromDocuments: 'Extract from Documents',
        loadFromUrl: 'Load from URL',
        database: 'Database',
        addData: 'Add Data',
        resetExtraction: 'Reset extraction',
        loadDataIn: 'Load data in',
        browser: 'Browser',
        disk: 'Disk',
    ...
    const getCurrentTabTitle = () => {
        const tabTitles: Record<UploadTabType, string> = {
            'menu': ui.loadData,
            'explore': ui.sampleDatasets,
            'upload': ui.uploadFile,
            'paste': ui.pasteData,
            'extract': ui.extractFromDocuments,
            'url': ui.loadFromUrl,
            'database': ui.database,
        };
        return tabTitles[activeTab] || ui.addData;
    };
        ...
        >
            <DialogTitle sx={{ display: 'flex', alignItems: 'center', gap: 1, pb: 1 }}>
                ...
                <Typography variant="h6" component="span">
                    {activeTab === 'menu' ? ui.loadData : getCurrentTabTitle()}
                </Typography>
                {activeTab === 'extract' && dataCleanBlocks.length > 0 && (
                    <Tooltip title={ui.resetExtraction}>
                        <IconButton 
                            size="small" 
                            color='warning' 
                            sx={{
                            ...
                            <ToggleButton value="browser">
                                <Tooltip title={`Data stays in browser only (limited to ${frontendRowLimit.toLocaleString()} rows)`} placement="bottom">
                                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                                            <LanguageIcon sx={{ fontSize: 14 }} /> {ui.browser}
                                        </Box>
                                    </Tooltip>
                                </ToggleButton>
                                ...
                                <Tooltip title={diskPersistenceDisabled
                                    ...
                                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                                        {serverConfig.WORKSPACE_BACKEND === 'azure_blob'
                                            ...
                                            : <><FolderOpenIcon sx={{ fontSize: 14 }} /> {ui.disk}</>}
                                    </Box>
                                </Tooltip>
                            </ToggleButton>
                        </ToggleButtonGroup>
                    ...
                    <Box sx={{
                        ...
                        {!serverConfig.DISABLE_FILE_UPLOAD ? (
                            ...
                            >
                                ...
                                <Typography variant={showFilePreview ? "body2" : "subtitle1"} gutterBottom>
                                    {appLanguage === 'fr' ? 'Glissez-deposez un fichier ici' : 'Drag & drop file here'}
                                </Typography>
                                <Typography variant="body2" color="text.secondary" sx={{ fontSize: showFilePreview ? '0.75rem' : '0.875rem' }}>
                                    {appLanguage === 'fr' ? 'ou ' : 'or '}<Link component="button" sx={{ textDecoration: 'underline', cursor: 'pointer' }}>{appLanguage === 'fr' ? 'Parcourir' : 'Browse'}</Link>
                                </Typography>
                                {!showFilePreview && (
                                    <Typography variant="caption" color="text.secondary" sx={{ mt: 1, display: 'block' }}>
                                        {appLanguage === 'fr' ? 'Formats pris en charge: CSV, TSV, JSON, Excel (xlsx, xls)' : 'Supported: CSV, TSV, JSON, Excel (xlsx, xls)'}
                                    </Typography>
                                )}
                            </Box>
                        ) : (
                            <Box sx={{ textAlign: 'center', py: 4, px: 2 }}>
                                <Typography color="text.secondary" sx={{ mb: 2 }}>
                                    {appLanguage === 'fr' ? 'Le telechargement de fichiers est desactive dans cet environnement.' : 'File upload is disabled in this environment.'}
                                </Typography>
                                <Typography variant="body2" color="text.secondary">
                                    {appLanguage === 'fr' ? 'Utilisez "Charger depuis une URL" pour charger des donnees depuis une source distante.' : 'Use "Load from URL" to load data from a remote source.'}
                                </Typography>
                            </Box>
                        )}
                        </Box>
                        ...
                        {showFilePreview && (
                            <Box sx={{ width: '90%', alignSelf: 'center' }}>
                                <MultiTablePreview
                                    ...
                                    emptyLabel={appLanguage === 'fr' ? 'Selectionnez un fichier pour previsualiser.' : 'Select a file to preview.'}
                        ...
                        {filePreviewTables && filePreviewTables.length > 0 && (
                            <Box sx={{ display: 'flex', justifyContent: 'center', gap: 1, alignItems: 'center' }}>
                                ...
                                >
                                    {appLanguage === 'fr' ? 'Charger la table' : 'Load Table'}
                                </Button>
                                {hasMultipleFileTables && (
                                    <Button
                                        ...
                                        sx={{ textTransform: 'none', width: 240 }}
                                    >
                                        {appLanguage === 'fr' ? 'Charger toutes les tables' : 'Load All Tables'}
                                    </Button>
                                )}
                            </Box>
                        )}
                        ...
                        <Box sx={{ width: '100%', maxWidth: showUrlPreview ? '80%' : 760, alignSelf: 'center', display: 'flex', flexDirection: 'column', gap: 2 }}>
                            ...
                            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.5 }}>
                                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                    <TextField
                                        fullWidth
                                        placeholder={appLanguage === 'fr' ? 'Entrez une URL: https://example.com/data.json ou /api/data' : 'Enter URL: https://example.com/data.json or /api/data'}
                                        value={tableURL || ''}
                                        onChange={(e) => setTableURL((e.target.value || '').trim())}
                                        inputRef={urlInputRef}
                                        error={tableURL !== "" && !hasValidUrl}
                                        helperText={tableURL !== "" && !hasValidUrl ? (appLanguage === 'fr' ? 'Entrez une URL valide commençant par http://, https:// ou /' : 'Enter a valid URL starting with http://, https://, or /') : undefined}
                                        size="small"
                                        sx={{ 
                                            flex: 1,
                                            '& .MuiInputBase-input': {
                                    ...
                                    >
                                        {appLanguage === 'fr' ? 'Previsualiser' : 'Preview'}
                                    </Button>
                                </Box>
                                <Typography variant="caption" color="text.secondary" sx={{ fontSize: '0.75rem', ml: 0.5 }}>
                                    {appLanguage === 'fr' ? "L'URL doit pointer vers des donnees au format CSV, JSON ou JSONL" : 'The URL must point to data in CSV, JSON, or JSONL format'}
                                </Typography>
                            </Box>
                                    ...
                                    <FormControlLabel
                                        ...
                                        label={
                                            <Typography component="span" variant="body2" sx={{ fontWeight: 500 }}>
                                                {appLanguage === 'fr' ? 'Mode surveillance' : 'Watch Mode'}
                                            </Typography>
                                        }
                                    />
                                    {urlAutoRefresh ? (
                                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, }}>
                                            <Typography variant="body2" color="text.secondary" sx={{ fontSize: '0.8rem' }}>
                                                {appLanguage === 'fr' ? 'verifier les mises a jour toutes les' : 'check data updates every'}
                                            </Typography>
                                            {[
                                                { seconds: 5, label: '5s' },
                                        ...
                                        {appLanguage === 'fr' ? "verifier et rafraichir automatiquement les donnees de l'URL a intervalles reguliers" : 'automatically check and refresh data from the URL at regular intervals'}
                                    </Typography>}
                            ...
                            {(!urlPreviewTables || urlPreviewTables.length === 0) && !urlPreviewLoading && (
                                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.5 }}>
                                    <Typography variant="caption" color="text.secondary" sx={{ mb: 0.5 }}>
                                        {appLanguage === 'fr' ? 'Essayer des exemples:' : 'Try examples:'}
                                    </Typography>
                                    <Box component="ul" sx={{ 
                                        listStyle: 'none', 
                                        padding: 0, 
                                            ...
                                            >
                                                ...
                                                {example.resetUrl && (
                                                    ...
                                                    >
                                                        ...
                                                        {appLanguage === 'fr' ? 'reinitialiser' : 'reset'}
                                                    </Typography>
                                                )}
                                            </Box>
                                        ))}
                        ...
                        {showUrlPreview && (
                            <Box sx={{ width: '90%', alignSelf: 'center' }}>
                                <MultiTablePreview
                                    ...
                                    emptyLabel={appLanguage === 'fr' ? 'Entrez une URL puis cliquez sur Previsualiser pour voir les donnees.' : 'Enter a URL and click Preview to see data.'}
                            ...
                            <Box sx={{ display: 'flex', justifyContent: 'center', gap: 1, alignItems: 'center' }}>
                                {urlAutoRefresh && (
                                    <Typography variant="caption" color="success.main" sx={{ mr: 1 }}>
                                        ...
                                        {appLanguage === 'fr' ? 'Mode surveillance' : 'Watch mode'}: {urlRefreshInterval < 60 ? `${urlRefreshInterval}s` : `${Math.floor(urlRefreshInterval / 60)}m`}
                                    </Typography>
                                )}
                                <Button
                                    variant="contained"
                    ...
                    <Box sx={{
                        ...
                        {isOverSizeLimit && (
                            <Box sx={{ 
                                ...
                                <Typography variant="caption" sx={{ flex: 1, color: 'error.main', fontWeight: 500 }}>
                                    ...
                                    {appLanguage === 'fr' ? "Veuillez utiliser l'onglet BASE DE DONNEES pour les gros jeux de donnees." : 'Please use the DATABASE tab for large datasets.'}
                                </Typography>
                            </Box>
                        )}
                        ...
                        {isLargeContent && !isOverSizeLimit && (
                            <Box sx={{ 
                                ...
                                <Typography variant="caption" sx={{ flex: 1 }}>
                                    {appLanguage === 'fr' ? 'Contenu volumineux detecte' : 'Large content detected'} ({Math.round(pasteContent.length / 1000)}KB). 
                                    {showFullContent
                                        ? (appLanguage === 'fr' ? 'Affichage complet (peut etre lent)' : 'Showing full content (may be slow)')
                                        : (appLanguage === 'fr' ? 'Affichage d’un apercu pour la performance' : 'Showing preview for performance')}
                                </Typography>
                                <Button 
                                    size="small" 
                                    variant="outlined" 
                                ...
                                >
                                    {showFullContent
                                        ? (appLanguage === 'fr' ? "Afficher l'apercu" : 'Show Preview')
                                        : (appLanguage === 'fr' ? 'Afficher tout' : 'Show Full')}
                                </Button>
                            </Box>
                        )}
                        ...
                        <Box sx={{ width: '100%', maxWidth: hasPasteContent ? 'none' : 720 }}>
                            <TextField
                                ...
                                placeholder={appLanguage === 'fr' ? 'Collez vos donnees ici (format CSV, TSV ou JSON)' : 'Paste your data here (CSV, TSV, or JSON format)'}
                                InputProps={{
                            ...
                            {isLargeContent && !showFullContent && (
                                <Box sx={{ 
                                    ...
                                    <Typography variant="caption" color="text.secondary" sx={{ fontSize: '0.7rem' }}>
                                        {appLanguage === 'fr'
                                            ? "Mode apercu: edition desactivee. Cliquez sur \"Afficher tout\" pour activer l'edition."
                        ...
                        <Box sx={{ display: 'flex', justifyContent: 'flex-end', mt: 2, gap: 1 }}>
                            <Button
                                ...
                                sx={{ textTransform: 'none' }}
                            >
                                {appLanguage === 'fr' ? 'Charger les donnees' : 'Upload Data'}
                            </Button>
                        </Box>
                    </Box>
                </TabPanel>
                ...
                <TabPanel value={activeTab} index="explore">
                    <Box sx={{ p: 2, height: '100%', boxSizing: 'border-box', display: 'flex', flexDirection: 'column' }}>
                        ...
                        handleSelectDataset={async (dataset) => {
                            ...
                            setDatasetLoadingLabel(appLanguage === 'fr' ? `Chargement de ${dataset.name}...` : `Loading ${dataset.name}...`);
            ...
            <Backdrop
                ...
                sx={{
                    ...
                    backgroundColor: (theme) => theme.palette.mode === 'dark' ? 'rgba(30, 30, 30, 0.95)' : 'rgba(255, 255, 255, 0.85)',
            ...
            >
                ...
                <Typography variant="body2" color="text.secondary">
                    {datasetLoadingLabel || (appLanguage === 'fr' ? 'Chargement des donnees...' : 'Loading data...')}
                </Typography>
                <Button
                    variant="text"
                    size="small"
