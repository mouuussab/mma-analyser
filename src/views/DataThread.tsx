...
import { getChartTemplate, getLocalizedChartType } from '../components/ChartTemplates';
                    ...
                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, flexWrap: 'nowrap' }}>
                            <FormControlLabel
                                ...
                                label={
                                    <Typography variant="body2" sx={{ fontSize: 11 }}>
                                        {language === 'fr' ? 'Surveiller les mises a jour' : 'Watch for updates'}
                                    </Typography>
                                }
                                sx={{ mr: 0 }}
                            />
                            ...
                            {onRefreshNow && (
                                ...
                                >
                                    {language === 'fr' ? 'Actualiser maintenant' : 'Refresh now'}
                                </Button>
                            )}
                        </Box>
                    </Box>
            ...
            <ClickAwayListener onClickAway={handleCancel}>
                ...
                >
                    <Typography variant="subtitle2" sx={{ mb: 1 }}>
                        {language === 'fr' ? 'Joindre des metadonnees a ' : 'Attach metadata to '}
                        <Typography component="span" sx={{ fontSize: 'inherit', color: 'primary.main'}}>{tableName}</Typography>
                    </Typography>
                    <TextField
                        autoFocus
                        label={language === 'fr' ? 'metadonnees' : 'metadata'}
                        placeholder={language === 'fr'
                            ? "Joignez des contextes ou des consignes supplementaires pour aider les agents IA a mieux comprendre et traiter les donnees."
                            : "Attach additional contexts or guidance so that AI agents can better understand and process the data."}
                        fullWidth
                        multiline
                        slotProps={{
            ...
            <ClickAwayListener onClickAway={onClose}>
                ...
                >
                    <Typography variant="subtitle2" sx={{ mb: 0.5, fontSize: 12 }}>
                        {language === 'fr' ? 'Renommer la table' : 'Rename table'}
                    </Typography>
                    <TextField
                        autoFocus
                        fullWidth
                    ...
                    <Box sx={{ mt: 0.5, display: 'flex', gap: 1, justifyContent: 'flex-end' }}>
                        <Button size="small" onClick={onClose}>{language === 'fr' ? 'Annuler' : 'Cancel'}</Button>
                        <Button size="small" onClick={handleSave} color="primary" disabled={name.trim() === '' || name.trim() === initialValue}>{language === 'fr' ? 'Enregistrer' : 'Save'}</Button>
                    </Box>
                </Paper>
            </ClickAwayListener>
        </Popper>
        ...
        <Box sx={{ ...sx,
            ...
            >
                ...
                >
                    ...
                    <Typography sx={{ fontSize: 11, fontWeight: 600 }}>{language === 'fr' ? 'ajouter des donnees' : 'add data'}</Typography>
                </Box>
            </Box>
        ...
        return (
            <Box key={`timeline-row-${item.key}`} sx={{ display: 'flex', flexDirection: 'row', position: 'relative', ...rowHighlightSx }}>
                <Box sx={{ 
                    ...
                    <Box sx={{ flexShrink: 0, zIndex: 1, backgroundColor: theme.palette.background.paper, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        {getTimelineDot(item)}
                    </Box>
                    {!isLast && (
        ...
        >
            ...
            {selectedTableForMenu?.derive != undefined && (
                ...
                >
                    ...
                    {selectedTableForMenu?.anchored
                        ? (language === 'fr' ? 'Detacher la table' : 'Unpin table')
                        : (language === 'fr' ? 'Epingler la table' : 'Pin table')}
                </MenuItem>
            )}
            ...
            {selectedTableForMenu?.derive == undefined && (
                ...
                >
                    ...
                    {selectedTableForMenu?.attachedMetadata
                        ? (language === 'fr' ? 'Modifier les metadonnees' : 'Edit metadata')
                        : (language === 'fr' ? 'Joindre des metadonnees' : 'Attach metadata')}
                </MenuItem>
            )}
             ...
             (selectedTableForMenu.source?.type === 'stream' || selectedTableForMenu.source?.type === 'database') && (
                ...
                >
                    ...
                    {selectedTableForMenu.source?.autoRefresh
                        ? (language === 'fr' ? "Parametres d'actualisation" : 'Refresh settings')
                        : (language === 'fr' ? 'Surveiller les mises a jour' : 'Watch for updates')}
                </MenuItem>
            )}
            ...
            {selectedTableForMenu?.derive == undefined && selectedTableForMenu?.source?.type !== 'database' && (
                ...
                >
                    ...
                    {language === 'fr' ? 'Remplacer les donnees' : 'Replace data'}
                </MenuItem>
            )}
            <MenuItem 
                onClick={(e) => {
            ...
            >
                ...
                {language === 'fr' ? 'Supprimer la table' : 'Delete table'}
            </MenuItem>
        </Menu>
...
const ChartThumbnail: FC<{
    ...
    const theme = useTheme();
    const language = useAppLanguage();
    const localizedChartType = getLocalizedChartType(chart.chartType, language);

    let deleteButton = <Box className='data-thread-chart-card-action-button'
        sx={{ zIndex: 10, color: theme.palette.mode === 'dark' ? theme.palette.primary.light : 'blue', position: "absolute", right: 1, background: theme.palette.mode === 'dark' ? 'rgba(30, 30, 30, 0.98)' : 'rgba(255, 255, 255, 0.95)' }}>
    ...
    if (['Auto', '?'].includes(chart.chartType)) {
        return <Box 
            ...
            sx={{ width: "100%", color: 'text.secondary', height: 48, display: "flex", backgroundColor: theme.palette.background.paper, position: 'relative', flexDirection: "column" }}>
            {pendingOverlay}
    ...
    if (status == 'unavailable' || chart.chartType == "Table") {
        ...
        return <Box key={`unavailable-${chart.id}`} width={"100%"}
            ...
            onClick={() => onChartClick(chart.id, table.id)}
            sx={{ display: "flex", backgroundColor: theme.palette.background.paper, position: 'relative', flexDirection: "column" }}>
            {pendingOverlay}
            ...
            >
                <Box sx={{ margin: "auto" }}>
                    ...
                    >
                        <img 
                            src={chart.thumbnail} 
                            alt={`${localizedChartType} ${language === 'fr' ? 'graphique' : 'chart'}`}
