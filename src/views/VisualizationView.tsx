...
import { CHART_TEMPLATES, getChartTemplate, getLocalizedChartType } from '../components/ChartTemplates';
...
const visText: Record<'en' | 'fr', {
    likeIt: string;
    notAnymore: string;
    delete: string;
    openVegaEditor: string;
    sampleAgain: string;
    derivedConcepts: string;
    dataTransformationCode: string;
    chartInsight: string;
    zoomOut: string;
    zoomIn: string;
}> = {
    en: {
        ...
        delete: "Delete",
        openVegaEditor: "Open in Vega Editor",
        sampleAgain: "sample again!",
        derivedConcepts: "Derived Concepts",
        dataTransformationCode: "Data transformation code",
    ...
    fr: {
        ...
        delete: "Supprimer",
        openVegaEditor: "Ouvrir dans l'éditeur Vega",
        sampleAgain: "rééchantillonner !",
        derivedConcepts: "Concepts dérivés",
        dataTransformationCode: "Code de transformation des données",
        chartInsight: "Insight du graphique",
        zoomOut: "dézoomer",
        zoomIn: "zoomer",
    }
};
...
export let generateChartSkeleton = (icon: any, width: number = 160, height: number = 160, opacity: number = 0.5, theme?: any) => (
    <Box width={width} height={height} sx={{ display: "flex" }}>
        {icon == undefined ?
            <AddchartIcon sx={{ color: theme?.palette?.text?.disabled || "lightgray", margin: "auto" }} /> :
            typeof icon == 'string' ?
...
export const ChartEditorFC: FC<{}> = function ChartEditorFC({}) {
    ...
    let saveButton = (
        <Tooltip key="save-copy-tooltip" title={focusedChart.saved ? text.notAnymore : text.likeIt}>
            <span>
    ...
    let deleteButton = (
        <Tooltip title={text.delete} key="delete-btn-tooltip">
            <span>
    ...
    let vegaEditorButton = (
        <Tooltip key="vega-editor-tooltip" title={text.openVegaEditor}>
            <span>
            ...
            <Box ref={explanationComponentsRef} sx={{width: "100%", mx: "auto"}}>
                ...
                <Collapse in={codeViewOpen}>
                    <Box sx={{minWidth: 440, maxWidth: 800, padding: "0px 8px", position: 'relative', margin: '8px auto'}}>
                        ...
                        <CodeExplanationCard
                            title={text.dataTransformationCode}
    ...
    let chartResizer = useMemo(() => <Stack spacing={1} direction="row" sx={{ 
        ...
        backgroundColor: theme.palette.mode === 'dark' ? 'rgba(30, 30, 30, 0.95)' : 'rgba(255, 255, 255, 0.9)',
        borderRadius: '4px',
    }} alignItems="center">
        <Tooltip key="zoom-out-tooltip" title={text.zoomOut}>
            <span>
        ...
        <Tooltip key="zoom-in-tooltip" title={text.zoomIn}>
            <span>
                        ...
                        {templates
                            ...
                            .map((t, index) => (
                                ...
                                >
                                    ...
                                    <Typography sx={{ ml: '4px', whiteSpace: "nowrap", fontSize: '10px', lineHeight: 1.2 }}>
                                        {t?.chart ? getLocalizedChartType(t.chart, appLanguage) : ''}
                                    </Typography>
                                </Button>
                            ))
                        }
            ...
            <Box sx={{  margin: "auto" }}>
                ...
                <Divider sx={{my: 3}} textAlign='left'>
                    <Typography sx={{fontSize: 12, color: "text.secondary"}}>
                        {appLanguage === 'fr' ? 'ou, commencez avec un type de graphique' : 'or, start with a chart type'}
                    </Typography>
                </Divider>
                {chartSelectionBox}
            </Box>
