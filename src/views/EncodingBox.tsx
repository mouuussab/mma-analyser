...
const ebText: Record<'en' | 'fr', {
    rerunSmartSort: string;
}> = {
    en: {
        rerunSmartSort: "rerun smart sort"
    },
    fr: {
        rerunSmartSort: "relancer le tri intelligent"
    }
};
    ...
    let radioLabel = (label: string | React.ReactNode, value: any, key: string, width: number = 80, disabled: boolean = false, tooltip: string = "") => {
        ...
        if (tooltip != "") {
            comp = <Tooltip key={`${key}-tooltip`} title={tooltip} arrow slotProps={{
                tooltip: {
                    sx: { bgcolor: theme.palette.mode === 'dark' ? 'rgba(50, 50, 50, 0.95)' : 'rgba(255, 255, 255, 0.95)', color: theme.palette.mode === 'dark' ? 'rgba(255,255,255,0.95)' : 'rgba(0,0,0,0.95)', border: `1px solid ${theme.palette.divider}` },
                },
            }}>{comp}</Tooltip>
        }
        return comp;
            ...
            if (autoSortResult != undefined && autoSortResult.length > 0) {
                ...
                let autoSortOpt = 
                    <Tooltip title={autoSortOptTitle} arrow componentsProps={{
                        tooltip: {
                          sx: {
                            bgcolor: theme.palette.mode === 'dark' ? 'rgba(50, 50, 50, 0.95)' : 'rgba(255, 255, 255, 0.95)',
                ...
                sortByOptions = [
                    ...
                    <FormControlLabel sx={{ width: 180, margin: 0 }} key={"auto"}
                        ...
                        label={<Box sx={{width: '100%', display:'flex'}}>
                                    {autoSortOpt}
                                    <Tooltip title={ebText[appLanguage].rerunSmartSort}>
    ...
    let createConceptInputBox = <Autocomplete
        ...
        renderOption={(props, option) => {
            ...
            if (fieldItem) {
                ...
                let draggleCardHeaderBgOverlay = theme.palette.mode === 'dark' ? 'rgba(30, 30, 30, 0.9)' : 'rgba(255, 255, 255, 0.9)';
                
                // Add subtle tint for non-focused fields
                if (activeTable && !activeTable.names.includes(fieldItem.name)) {
                    draggleCardHeaderBgOverlay = theme.palette.mode === 'dark' ? 'rgba(30, 30, 30, 1)' : 'rgba(255, 255, 255, 1)';
                }
                    ...
                    <Box ref={drop} className="channel-encoded-field">
                        <IconButton //className="encoding-shelf-action-button"
                            ...
                            size="small" sx={{
                                ...
                                position: "relative", borderRight: `1px solid ${theme.palette.divider}`, width: '64px', 
