...
import { channelGroups, CHART_TEMPLATES, getChartChannels, getChartTemplate, getLocalizedChartType } from '../components/ChartTemplates';
...
const escText: Record<'en' | 'fr', {
    formulateAndOverride: string;
    formulate: string;
}> = {
    en: {
        formulateAndOverride: "formulate and override",
        formulate: "Formulate"
    },
    fr: {
        formulateAndOverride: "formuler et remplacer",
        formulate: "Formuler"
    }
};
...
export const EncodingShelfCard: FC<EncodingShelfCardProps> = function ({ chartId }) {
    ...
    let formulateInputBox = <Card key='text-input-boxes' variant='outlined' sx={{
        ...
        <TextField
            ...
            value={prompt}
            placeholder={language === 'fr' ? 'poursuivre sur ce graphique' : 'follow up on this chart'}
            fullWidth
            multiline
            minRows={2}
            maxRows={5}
        ...
        <Box sx={{
            ...
            <Tooltip title={currentChartIdeas.length > 0 ? (language === 'fr' ? "Actualiser les idees" : "Refresh ideas") : (language === 'fr' ? "Obtenir des idees" : "Get ideas")}>
                <span>
            ...
            {trigger ? 
                <Tooltip title={<Typography sx={{fontSize: 11}}>{escText[language].formulateAndOverride} <TableIcon sx={{width: 10, height: 10, marginBottom: '-1px'}}/>{trigger.resultTableId}</Typography>}>
                    <span>
                ...
                : 
                <Tooltip title={escText[language].formulate}>
                    <span>
                        ...
                        renderValue={(value: string) => {
                            ...
                            return (
                                <div style={{display: 'flex', padding: "0px 0px 0px 4px"}}>
                                    ...
                                    <ListItemText sx={{marginLeft: "2px", whiteSpace: "initial"}} slotProps={{primary: {fontSize: 12}}}>
                                        {t?.chart ? getLocalizedChartType(t.chart, language) : ''}
                                    </ListItemText>
                                </div>
                            )
                        }}
                                    ...
                                    >
                                        <Box sx={{display: 'flex'}}>
                                            ...
                                            <ListItemText 
                                                ...
                                                sx={{ margin: 0 }}
                                            >
                                                {getLocalizedChartType(t.chart, language)}
                                            </ListItemText>
                                        </Box>
                                    </MenuItem>
                                ))
