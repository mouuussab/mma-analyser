...
export function useFormulateData() {
    ...
    const appLanguage = (typeof document !== 'undefined' && document.body?.getAttribute('data-language') === 'fr') ? 'fr' : 'en';
    ...
    async function streamIdeas(options: StreamIdeasOptions): Promise<void> {
        ...
        try {
                ...
                try {
                    ...
                    if (errorText) {
                        try {
                            ...
                            errorDetails = errorJson.error || errorJson.error_message || errorJson.message || errorText;
                        } catch {
                            errorDetails = errorText.slice(0, 200);
                        }
                    }
                } catch (readErr) {
                    console.warn('Could not read error response body:', readErr);
                }
            ...
            const extractJsonBlocks = (text: string): any[] => {
                const blocks: any[] = [];
                let depth = 0;
                let inString = false;
                let escape = false;
            ...
            const updateState = (currentLines: string[]) => {
                let dataBlocks = currentLines
                    .map(line => { try { return JSON.parse(line.trim()); } catch (e) { return null; } })
                    .filter(block => block != null);

                // Fallback for local models that don't follow "data: {json}" line format exactly.
                if (dataBlocks.length === 0 && rawStream.trim()) {
                    dataBlocks = extractJsonBlocks(rawStream);
                }
            ...
            if (lines.length === 0 && extractJsonBlocks(rawStream).length === 0) {
                throw new Error('No valid results returned from agent');
            }
        } catch (error) {
            console.error('[streamIdeas] Error caught:', error, typeof error, error instanceof Error);
            ...
            console.error('[streamIdeas] Final error detail:', errorDetail);
            ...
            dispatch(dfActions.addMessages({
                ...
                "value": isAbortError
                    ? (appLanguage === 'fr'
                        ? "La generation d'idees a expire. Reessayez ou augmentez le delai dans Parametres."
                        : "Idea generation timed out. Please retry or increase timeout in Settings.")
                    : (appLanguage === 'fr'
                        ? "Impossible d'obtenir des idees depuis l'agent d'exploration. Reessayez."
